...
class RowSegmentatorNew(BaseRowSegmentator):
    #K�pen a sorok szegment�l�sa
    #�gy �ri el, hogy a v�zszintes vet�leten ahol van 0 �rt�k, ott van sort�r�s
    def row_segmentation(self, image):
        ...
        for i in range(1, num_labels):
            ...
            letter_pixels = cv2.findNonZero(row_image_inverted) #Megkeresi a sorban a sz�veg befoglal� t�glalapj�t
            x, y, w, h = cv2.boundingRect(letter_pixels)
            
            row_image = row_image[y:y+h, x:x+w] #Kiv�gja a sz�veget bel�le
